...
export type PhoneLocationStatus =
    ...
    | 'UNKNOWN_ERROR'
    /** A reading is being sent to the backend. */
    | 'PUBLISHING'
...
/** The backend accepted the reading. */
 ...
 * is worth another attempt with the same reading.
 */
...
/** No bus is signed in, so nothing can be published. */
 ...
 *
 * Covers publishing as well as the GPS read: a driver tapping twice must not
...
export function describePhoneLocationState(state: PhoneLocationState): PhoneLocationView {
    switch (state.status) {
        ...
        case 'NOT_SIGNED_IN':
            // Retrying cannot fix this one, so it does not offer a retry.
            return {
                tone: 'warning',
